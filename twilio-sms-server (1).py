...
class nexmo_search:
    ...
    def GET(self):
        ...
        if filenames != '':
            ...
            thread = get("http://128.122.140.120:8080/search_and_get_random", get_data)
            thread.start()
...
class nexmo_get:
    ...
    def POST(self):
        syslog.syslog("RAPID: Inside Nexmo Get")
        data = web.input(myfile={})
        filedir = '/home/talal/GetFiles'
        if 'myfile' in data: 
...
class nexmo_sms(incoming_sms):
    ...
    def GET(self):
        ...
        if all(i in data for i in needed_fields):
            ...
            syslog.syslog("RAPID: post sms ret:%s,%s" %(body,str(time.time())));
...
if __name__ == "__main__":
    syslog.syslog("RAPID: twilio server started")
    app.run()
